...
public abstract class AuthScheme<SpecT> implements Serializable {
    ...
    /**
     * Represents an {@link AuthScheme} which uses a {@link dev.getelements.elements.rt.manifest.http.Header} to provide credentials.
     */
    public static class Header extends AuthScheme<dev.getelements.elements.rt.manifest.http.Header> {

        private dev.getelements.elements.rt.manifest.http.Header spec;

        @Override
        public dev.getelements.elements.rt.manifest.http.Header getSpec() {
            return spec;
        }

        @Override
        public void setSpec(dev.getelements.elements.rt.manifest.http.Header spec) {
            this.spec = spec;
        }
    }
