...
public class StandardCryptoKeyUtility implements CryptoKeyUtility {
    ...
    @Override
    public X509KeyPair generateKeyPair(final AuthSchemeAlgorithm authSchemeAlgorithm) {
    ...
    private X509KeyPair generate(final LockedPair<KeyPairGenerator> generator) {
        try (var _m = generator.enter()) {
            final var keyPair = generator.get().generateKeyPair();
            return new X509KeyPair(keyPair);
        }
    }
