...
public class AnonMetadataService implements MetadataService {

    private MetadataDao metadataDao;

    @Override
    public Metadata getMetadataObject(final String metadataId) {
        return metadataDao.getMetadata(metadataId, User.Level.UNPRIVILEGED);
    }

    @Override
    public Pagination<Metadata> getMetadataObjects(final int offset, final int count) {
        return metadataDao.getMetadatas(offset, count, User.Level.UNPRIVILEGED);
    }

    @Override
    public Pagination<Metadata> getMetadataObjects(final int offset, final int count, final String search) {
        return metadataDao.searchMetadatas(offset, count, search, User.Level.UNPRIVILEGED);
    }
