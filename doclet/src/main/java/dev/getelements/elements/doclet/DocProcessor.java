...
import dev.getelements.elements.doclet.lua.LDocStubProcessorExpose;
import dev.getelements.elements.doclet.lua.LDocStubProcessorIntrinsic;
...
import static dev.getelements.elements.doclet.DocAnnotations.*;
...
public interface DocProcessor<StubT extends DocRoot> {
    ...
    static List<DocProcessor<?>> get(final DocContext cxt, final TypeElement typeElement) {
        ...
        if (intrinsic != null) {
            skipStandardProcessing = true;
            processors.add(new LDocStubProcessorIntrinsic(cxt, intrinsic));
        }
