...
public interface GitApplicationAssetLoader extends ApplicationAssetLoader {
     ...
     * branches or commits to be used as the reference for the {@link DeploymentId} that is
     * associated with the code.
     */
    String DEFAULT_MAIN_BRANCH = "main";

    /**
     * Opens a {@link Git} instance for the supplied {@link DeploymentId} which can be used to manipulate the various
     * files within the repository.  When call returns the supplied {@link Git} instance will be closed.
     *
     * {@see {@link DeploymentId#forUniqueName(String)}} to understand how this method interprets the first parameter.
     *
     * @param applicationUniqueName the {@link DeploymentId} for which to open a {@link Git} instance
    ...
    default void performInGit(final String applicationUniqueName, final BiConsumer<Git, Path> gitConsumer) {
        final var id = DeploymentId.forUniqueName(applicationUniqueName);
     ...
     * files within the repository.  When call returns the supplied {@link Git} instance will be closed.
     *
     * @param deploymentId the {@link DeploymentId} for which to open a {@link Git} instance
     * @param gitConsumer consumes an instance of {@link Git} which will be used to perform the desired actions
     */
    void performInGit(DeploymentId deploymentId, BiConsumer<Git, Path> gitConsumer);
