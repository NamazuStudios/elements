...
public class StandardProxyBuilder<ProxyT> implements ProxyBuilder<ProxyT> {
    ...
    public ProxyBuilder<ProxyT> withSharedMethodHandleCache() {
        return withMethodHandleCache((methodHandleKey, methodHandleSupplier) -> {
            final Map<MethodHandleKey, MethodHandle> cache = SharedMethodHandleCache.getSharedMethodHandleCache();
            return cache.computeIfAbsent(methodHandleKey, k -> methodHandleSupplier.get());
        });
    }
