...
import static dev.getelements.elements.rt.transact.unix.UnixFSUtils.LinkType.*;
...
public class UnixFSGarbageCollectionCycle {
    ...
    private final SortedMap<dev.getelements.elements.rt.Path, Operation> pathOperations = new TreeMap<>(reverseOrder(this::sort));
    ...
    private class CollectionPassExecutionHandler implements ExecutionHandler {
        ...
        private void collectAncestorDirectories(final dev.getelements.elements.rt.Path rtPath) {
            for (dev.getelements.elements.rt.Path parent = rtPath.parent(); !parent.isRoot(); parent = parent.parent()) {
