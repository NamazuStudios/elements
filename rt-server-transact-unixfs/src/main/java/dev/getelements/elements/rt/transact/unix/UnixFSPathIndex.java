...
import dev.getelements.elements.rt.transact.RevisionMap;
...
import static dev.getelements.elements.rt.transact.unix.UnixFSUtils.LinkType.REVISION_SYMBOLIC_LINK;
...
public class UnixFSPathIndex implements PathIndex {
    ...
    @Override
    public RevisionMap<dev.getelements.elements.rt.Path, ResourceId> getRevisionMap(
            final NodeId nodeId) {
        return new PathRevisionMap(nodeId);
    }

    @Override
    public RevisionMap<ResourceId, Set<dev.getelements.elements.rt.Path>> getReverseRevisionMap(
            final NodeId nodeId) {
    ...
    private class PathRevisionMap implements RevisionMap<dev.getelements.elements.rt.Path, ResourceId> {
        ...
        public Revision<ResourceId> getValueAt(final Revision<?> revision,
                                               final dev.getelements.elements.rt.Path key) {
    ...
    private class ReversePathRevisionMap implements RevisionMap<ResourceId, Set<dev.getelements.elements.rt.Path>> {
        ...
        @Override
        public Revision<Set<dev.getelements.elements.rt.Path>> getValueAt(final Revision<?> revision,
