...
import dev.getelements.elements.rt.transact.RevisionDataStore.PendingRevisionChange;
...
public class JournalTransactionalResourceServicePersistenceEnvironment implements
    ...
    @Override
    public dev.getelements.elements.rt.transact.ExclusiveReadWriteTransaction openExclusiveRW(final NodeId nodeId) {
        try {
            exclusiveLock.lock();
            final var entry = getTransactionJournal().newMutableEntry(nodeId);
            final var revision = getRevisionDataStore().lockLatestReadCommitted();
