...
public class JavaModule {
	...
	/**
	 * The name of the module as a constant.
	 */
	...
	private final NamedJavaFunction[] functions = { new Require(), new New(),
			...
			new Methods(), new Properties(), new Pcall() };
	...
	public void open(LuaState luaState) {
		luaState.register(NAME, functions, true);
	}
	...
	private static class Pairs implements NamedJavaFunction {
		// -- NamedJavaFunction methods
		@Override
		public int invoke(LuaState luaState) throws Throwable {
			luaState.checkArg(
					1,
	...
	private static class Pcall implements NamedJavaFunction {
		...
		public int invoke(final LuaState luaState) {
			...
			// Discards anything but the first argument, just in case somebody passed too many arguments to the
			// method.  We also ensure that the first argument is actually a function.
			...
			try {
				...
				luaState.pushJavaObject(cause == null ? ex : ex.getCause());
				return 2;
			} catch (Exception ex) {
				luaState.setTop(0);
				luaState.pushBoolean(false);
				luaState.pushJavaObject(ex);
				return 2;
			}
