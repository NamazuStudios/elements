...
import com.namazustudios.socialengine.dao.mongo.model.blockchain.MongoBscSmartContract;
import com.namazustudios.socialengine.exception.blockchain.BscSmartContractNotFoundException;
...
public class MongoBscSmartContractDao implements BscSmartContractDao {
    ...
    @Override
    public Pagination<ElementsSmartContract> getBscSmartContracts(int offset, int count, String search) {
        final String trimmedSearch = nullToEmpty(search).trim();
        final Query<MongoBscSmartContract> mongoQuery = getDatastore().find(MongoBscSmartContract.class);
    ...
    @Override
    public ElementsSmartContract getBscSmartContract(String contractIdOrName) {
        final var objectId = getMongoDBUtils().parseOrReturnNull(contractIdOrName);

        var mongoContract = getDatastore().find(MongoBscSmartContract.class)
                .filter(Filters.or(
        ...
        if(mongoContract == null) {
            throw new BscSmartContractNotFoundException("Unable to find contract with an id or name of " + contractIdOrName);
        }
    ...
    @Override
    public ElementsSmartContract patchBscSmartContract(PatchSmartContractRequest patchSmartContractRequest) {
        getValidationHelper().validateModel(patchSmartContractRequest, ValidationGroups.Insert.class);

        final var query = getDatastore().find(MongoBscSmartContract.class);
    ...
    @Override
    public void deleteBscSmartContract(String contractId) {
        final var objectId = getMongoDBUtils().parseOrThrow(contractId, BscSmartContractNotFoundException::new);

        final var result = getDatastore()
                .find(MongoBscSmartContract.class)
                .filter(eq("_id", objectId))
                .delete();

        if(result.getDeletedCount() == 0){
            throw new BscSmartContractNotFoundException("BscSmartContract not deleted: " + contractId);
        }
    }

    private ElementsSmartContract transform(MongoBscSmartContract token) {
