...
import com.namazustudios.socialengine.jnlua.util.AbstractTableList;
import com.namazustudios.socialengine.jnlua.util.AbstractTableMap;
...
public class DefaultConverter implements Converter {
	...
	static {
		...
		final JavaObjectConverter<Number> numberConverter = (luaState, number) -> {
			if (number.doubleValue() == floor(number.doubleValue())) {
				luaState.pushInteger(number.intValue());
			} else {
	...
	public <T> T convertLuaValue(LuaState luaState, int index,
		...
		case NUMBER:
			...
			if (formalType == Object.class) {
				final double dVal = luaState.toNumber(index);
				if (floor(dVal) == dVal) {
					return (T) Integer.valueOf(luaState.toInteger(index));
				} else {
					return (T) Double.valueOf(dVal);
				}
			}
			break;
		case STRING:
	...
	public void convertJavaObject(LuaState luaState, Object object) {
		...
		final JavaObjectConverter<Object> javaObjectConverter = (JavaObjectConverter<Object>) JAVA_OBJECT_CONVERTERS.get(object.getClass());
		...
		if (object instanceof LuaValueProxy) {
			...
			final LuaValueProxy luaValueProxy = (LuaValueProxy) object;
