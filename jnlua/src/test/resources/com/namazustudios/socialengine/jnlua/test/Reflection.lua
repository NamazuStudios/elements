...
function module.testReflection ()
	-- Static field
	local TestObject = java.require("com.namazustudios.socialengine.jnlua.test.fixture.TestObject")
...
function module.testMethodDispatch ()
	-- Subclass test
	local TestObject = java.require("com.namazustudios.socialengine.jnlua.test.fixture.TestObject")
	local testObject = TestObject:new()
	local Sub = java.require("com.namazustudios.socialengine.jnlua.test.fixture.TestObject$Sub")
	local sub = Sub:new()
	assert(testObject:overloadedSub(testObject) == "super")
	assert(testObject:overloadedSub(sub) == "sub")
	
	-- Subinterface test
	local AB = java.require("com.namazustudios.socialengine.jnlua.test.fixture.TestObject$AB")
	local ab = AB:new()
	local AC = java.require("com.namazustudios.socialengine.jnlua.test.fixture.TestObject$AC")
	local ac = AC:new()
	local BC = java.require("com.namazustudios.socialengine.jnlua.test.fixture.TestObject$BC")
	local bc = BC:new()
...
-- VarArgs method test
function module.testVarargs ()
	local String = java.require("java.lang.String")
	assert(String:format("%s%.0f", "test", 1.1) == "test1")
	assert(true)
end
