...
export const MODEL_DEFINITIONS: Record<string, ModelSchema> = {
  ...
  'user/User': {
    ...
    fields: [
      ...
      { name: 'displayName', type: 'string', required: false, isArray: false, isMap: false, readOnly: true, description: "The user's display name, as reported by an OIDC provider. Only filled in if previously blank; never overwritten." },
  ...
  'auth/OAuth2AuthScheme': {
    ...
    fields: [
      { name: 'id', type: 'string', required: false, isArray: false, isMap: false, validationGroups: { insert: 'null', create: 'null', update: 'notNull' } },
      { name: 'name', type: 'string', required: true, isArray: false, isMap: false, description: 'Unique name for this OAuth2 auth scheme.' },
      { name: 'validationUrl', type: 'string', required: true, isArray: false, isMap: false, description: 'The URL to validate the OAuth2 token against.' },
      { name: 'method', type: 'enum', required: true, enumValues: ['GET', 'POST'], isArray: false, isMap: false, description: 'HTTP method for the validation request.' },
      { name: 'bodyType', type: 'enum', required: false, enumValues: ['NONE', 'FORM_URL_ENCODED', 'JSON'], isArray: false, isMap: false, conditionalVisibility: { dependsOn: 'method', showWhen: 'POST' }, description: 'How to encode the request body when using POST.' },
      { name: 'body', type: 'object', required: false, isArray: true, isMap: false, conditionalVisibility: { dependsOn: 'method', showWhen: 'POST' }, description: 'Body parameters for the validation request (POST only). Check User ID to mark which parameter contains the user identifier.' },
      { name: 'headers', type: 'object', required: false, isArray: true, isMap: false, description: 'HTTP headers to include in the validation request. Check User ID to mark which header contains the user identifier.' },
      { name: 'params', type: 'object', required: false, isArray: true, isMap: false, description: 'Query parameters to include in the validation request. Check User ID to mark which parameter contains the user identifier.' },
      { name: 'validStatusCodes', type: 'integer', required: false, isArray: true, isMap: false, description: 'HTTP status codes considered valid. Defaults to [200].' },
      { name: 'responseIdMapping', type: 'string', required: false, isArray: false, isMap: false, description: 'Determines how to map the user id in the response. Will search the response for the corresponding key. For example, if the response is {"response": {"params": {"steamid": <id>}}}, then input "steamid". Ignored if one of the parameters is marked as user id.' },
      { name: 'responseValidMapping', type: 'string', required: false, isArray: false, isMap: false, description: 'Optional key in the response whose value indicates whether the token/user is valid. For example: "is_valid", "active", or "success". If not set, only the HTTP status code is used.' },
  ...
  'auth/OidcProviderConfiguration': {
    ...
    fields: [
      { name: 'id', type: 'string', required: false, isArray: false, isMap: false, validationGroups: { insert: 'null', create: 'null', update: 'notNull' } },
      { name: 'name', type: 'string', required: true, isArray: false, isMap: false, description: "A unique, lowercase, URL-safe identifier for the provider (e.g. 'twitch'). Used as the 'provider' path segment and request body value for the OIDC session endpoints." },
      ...
      { name: 'extraAuthorizeParams', type: 'object', required: false, isArray: false, isMap: true, description: "Additional provider-specific query parameters to include in the authorize request. Add each parameter as its own row; a value that is itself a nested object or array (for example Twitch's 'claims') is edited as structured key/value rows. See the OIDC login docs for a full Twitch example." },
      ...
      { name: 'successRedirectUrl', type: 'string', required: false, isArray: false, isMap: false, description: 'Optional. If set, GET /oidc/{provider}/callback redirects the browser here on a successful login instead of rendering the default success page. Server-authoritative: not overridable per login attempt.' },
      { name: 'errorRedirectUrl', type: 'string', required: false, isArray: false, isMap: false, description: 'Optional. If set, GET /oidc/{provider}/callback redirects the browser here on a failed login instead of rendering the default error page. Server-authoritative: not overridable per login attempt.' },
      { name: 'adminLoginEnabled', type: 'boolean', required: false, isArray: false, isMap: false, description: "When enabled, this provider is offered as a login option on the admin panel's login page, in addition to normal player login. A user logging in this way still lands with standard (non-privileged) permissions; elevation to SUPERUSER is never automatic." },
      { name: 'displayName', type: 'string', required: false, isArray: false, isMap: false, conditionalVisibility: { dependsOn: 'adminLoginEnabled', showWhen: true }, description: "Human-readable name shown on the login button (e.g. 'Google', 'Sign in with Twitch'). Falls back to name if unset. Each provider has its own brand guidelines, so this is entirely up to you rather than derived by Elements." },
      { name: 'iconUrl', type: 'string', required: false, isArray: false, isMap: false, conditionalVisibility: { dependsOn: 'adminLoginEnabled', showWhen: true }, description: "URL of an icon/logo to show on this provider's login button, alongside displayName. Must be publicly fetchable with no authentication, since it's rendered on the login page before any session exists." },
    ],
  },
